...
namespace CloudDrive.Service.SkyDrive
{
	public class SkyDriveCloudService : ICloudService
	{
		...
		public CloudFile Get(string remotePathOrId)
		{
			...
			if (remoteFileInfo.Created_Time == null)
				return null;

			return new CloudFile()
			{
				RemotePath = remoteFileInfo.Upload_Location,
				FileType = remoteFileInfo.Type.ToLower() == "Folder" ? CloudFileType.Folder : CloudFileType.File,
				RemoteDateUpdated = remoteFileInfo.Updated_Time != null ? DateTime.Parse(remoteFileInfo.Updated_Time) : DateTime.MinValue,
				RemoteDateCreated = remoteFileInfo.Created_Time != null ? DateTime.Parse(remoteFileInfo.Created_Time) : DateTime.MinValue,
		...
		public CloudFile Set(CloudFile cloudFile)
		{
			...
			if (newRemoteFile != null)
			{
				...
				retFile.RemoteDateCreated = string.IsNullOrEmpty(newRemoteFile.Created_Time) ? DateTime.MinValue : DateTime.Parse(newRemoteFile.Created_Time);
				retFile.RemoteDateUpdated = string.IsNullOrEmpty(newRemoteFile.Updated_Time) ? DateTime.MinValue : DateTime.Parse(newRemoteFile.Updated_Time);
